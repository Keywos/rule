...
import {
  ...
  getIndexStats, IndexStats, cancelBuildIndex,
  closeDatabase,
...
function HighlightedText({ text, query }: {
  ...
  if (!query.trim()) {
    return <Text font={12} foregroundStyle="secondaryLabel" multilineTextAlignment="leading">{text}</Text>
  }
  ...
  return (
    <Text font={12} multilineTextAlignment="leading" styledText={{
      content: segments
    }} />
  )
}
...
export function SearchPanel({
    ...
    ;(async () => {
      try {
        ...
        if (stats.total === 0) {
          ...
          await buildIndex(dirPath, (count) => { setIndexingCount(count) })
  ...
  const buildDeepSearchIndex = async (forceRebuild: boolean = false) => {
    ...
    try {
      await buildIndex(dirPath, (count, _currentPath) => {
  ...
  const resultsSection = useMemo(() => {
    ...
    const navigateToFile = (result: DeepSearchResult, specificLine?: number) => {
      ...
      setTimeout(() => { navigatingRef.current = null }, 500)
    ...
    return (
      <Section title={`深度搜索找到 ${deepSearchResults.reduce((s, r) => s + ((r.allMatches?.length || 0) || (r.matchedLine ? 1 : 0)), 0)} 个匹配`}>
        {deepSearchResults.map((result) => {
          const matches = result.allMatches && result.allMatches.length > 0
            ? result.allMatches
            : result.matchedLine != null
              ? [{ line: result.matchedLine, content: result.matchedContent || '' }]
              : []
          ...
          return (
            <Group key={result.path}>
              {hasMatches ? (
                <Button action={() => toggleExpanded(result.path)} contextMenu={{
                  menuItems: <Group>
                ...
                >
                  <HStack spacing={12} alignment="center" padding={{ vertical: 6, leading: -9, trailing: 6 }}>
                    <Image systemName="chevron.right" frame={{ width: 12, height: 12 }} foregroundStyle="tertiaryLabel" rotationEffect={{ degrees: isExpanded ? 90 : 0, anchor: 'center' }} animation={{ animation: Animation.default(), value: isExpanded }} />
                    <Image systemName={result.icon} frame={{ width: 20, height: 20 }} foregroundStyle={result.iconColor as any} />
                    <VStack alignment="leading" spacing={0}>
                      <Text font="body" lineLimit={1}>{result.name}</Text>
                      <Text font="caption2" foregroundStyle="quaternaryLabel" lineLimit={1}>{result.relativePath}</Text>
                    </VStack>
                    <Spacer />
                    <Text font="caption" foregroundStyle="tertiaryLabel">{matches.length}行</Text>
                  </HStack>
                </Button>
              ) : (
                <Button action={() => navigateToFile(result)} contextMenu={{
                  menuItems: <Group>
                    <Button title="跳转到目录" action={() => gotoParentDir(result)} />
                    <Button title="删除" role="destructive" action={() => deleteFile(result)} />
                    <Button title="拷贝" action={() => copyToStorage(result)} />
                ...
                >
                  <HStack spacing={12} alignment="center" padding={{ vertical: 6, leading: 15, trailing: 6 }}>
                    ...
                    <Spacer />
                    <Text font="caption" foregroundStyle="tertiaryLabel">0行</Text>
                  </HStack>
                </Button>
              )}
        ...
        <Group key="load-more">
          <Button action={() => { if (!isLoadingMore) loadMoreResults() }}>
            <HStack padding={{ vertical: 12, horizontal: 16 }} alignment="center">
              {isLoadingMore ? (
                <Text font="caption" foregroundStyle="tertiaryLabel">加载中…</Text>
              ) : (
                <Text font="caption" foregroundStyle="secondaryLabel">加载更多</Text>
              )}
            </HStack>
          </Button>
        </Group>
