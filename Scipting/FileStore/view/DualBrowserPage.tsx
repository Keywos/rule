...
export function DualBrowserPage({
  ...
  useEffect(() => {
    setLeftSettings(prev => ({ ...prev, isFullscreen: settings.isFullscreen, showExitButton: settings.showExitButton }))
    setRightSettings(prev => ({ ...prev, isFullscreen: settings.isFullscreen, showExitButton: settings.showExitButton }))
  }, [settings])
  ...
  const handleLeftSettingsChange = (newSettings: AppSettings) => {
    ...
    // 非导航类变更传播给父级（如全屏）
    if (onSettingsChange && (newSettings.isFullscreen !== settings.isFullscreen)) {
  ...
  // 把文件复制到右侧当前目录
  const handleCopyLeftToRight = useCallback(async (filePath: string) => {
    if (!rightDir) {
  ...
  // 把文件复制到左侧当前目录
  const handleCopyRightToLeft = useCallback(async (filePath: string) => {
    if (!leftDir) {
      ...
      toast={{
        ...
        content: (
          <HStack spacing={8}>
            ...
            <Text foregroundStyle="white" font={13}>{copyToastMessage}</Text>
          </HStack>
        ),
        position: "top",
      }}>
          ...
          return (
            <ZStack>
              ...
              <DraggableDivider
                ...
                totalH={totalH}
                ratio={ratio}
                onDragEnd={(newRatio) => setRatio(newRatio)}
...
function DraggableDivider({
  ...
  const previewOffset = layoutDir === 'horizontal'
