...
import { ImageViewer, VideoViewerPage, FileNavigationDest } from "./MediaViewer";
...
import {
  ensureDir,
  makeTimestamp,
  importSinglePhotoResult,
} from "../manager/importHelpers";
import {
  DROP_ACCEPTED_TYPES,
  handleDropToDirectory,
} from "../manager/dropHandler";
  ...
  const handleDelete = async () => {
    try {
      ...
      withAnimation(Animation.smooth({ duration: 0.35 }), () => {
        if (onDeleteFile) { onDeleteFile(file.path); }
        else { onRefresh(); }
      });
    } catch (e) {
      console.log("删除失败:", e);
      ...
      <Button
        ...
        trailingSwipeActions={{
          actions: [
            ...
            <Button title="简介" action={handleShowInfo} />,
          ]
        }}
        leadingSwipeActions={{
          ...
          menuItems: (
            <Group>
              ...
              {getFileCategory(file.extension) === 'archive' ? <>
                ...
                <Button title={`解压到${sanitizeExtractDirName(file.name)}`} systemImage="folder.badge.gearshape" action={async () => {
                  try {
                    const archiveName = sanitizeExtractDirName(file.name)
        ...
        menuItems: (
          <Group>
                ...
                try {
                  ...
                  if (data) {
                    ...
                    if (unpacked) {
                      ...
                      try { FileManager.remove(imgTmp); } catch {}
                      try { FileManager.remove(vidTmp); } catch {}
                    }
                  }
                } catch (e) {
            ...
            {!file.isDirectory && getFileCategory(file.extension) === 'archive' ? <>
              ...
              <Button title={`解压到${sanitizeExtractDirName(file.name)} 内`} systemImage="folder.badge.gearshape" action={async () => {
                try {
                  const archiveName = sanitizeExtractDirName(file.name)
                  let extractDir = Path.join(dirPath || Path.dirname(file.path), archiveName)
                  // 避免覆盖已有目录，追加 _01 _02
...
function FileContentView({ file, parentFullscreen }: { file: FileInfo; parentFullscreen?: boolean }) {
  ...
  if (content != null) {
    return (
      <FilePreviewView fileInfo={file} content={content} />
    )
  }
  ...
  return (
    <NavigationStack>
      <List navigationTitle={file.name} navigationBarTitleDisplayMode="inline">
        <Section>
          <HStack spacing={12} padding={{ vertical: 8 }}>
            <Image systemName={file.icon} frame={{ width: 40, height: 40 }} foregroundStyle={file.iconColor} />
            <VStack alignment="leading">
              <Text font="headline">{file.name}</Text>
              <Text font="caption" foregroundStyle="secondaryLabel">
                {fmtSize(file.size)} · {fmtDate(file.modificationDate)}
              </Text>
            </VStack>
          </HStack>
        </Section>
...
function GeneralBrowser({ dirPath = '', dirName, rootPath, rootName, parentFullscreen, navPath: outerNavPath, navigationDestination, items, onItemsChange, toolbarOtherItems, showFolderItemCounts, onOpenSettings, initialSortOrder, initialFilterType, onSortFilterChange, refreshKey, highlightFile, isHomePage, settings, onSettingsChange, onFullscreen, bookmarks, externalCopiedPath, onExternalCopy, oppositeDirName, onCopyToOppositeDir, onDirChange, initialLoadDelay, clipboardSyncTrigger, addFilesRef, onFilesAdded, onDropCompleted, onFolderCountChanged, folderCountUpdateRef }: {
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (prev !== null) {
          ...
          const changed = current.length !== newList.length ||
            current.some((f, i) => f.path !== newList[i].path || f.size !== newList[i].size)
          if (changed) {
            setFiles(newList)
          }
        }
  ...
  useEffect(() => {
    ...
    if (match) {
      ...
      console.log('no match found for:', highlightFile, 'first few files:', displayFiles.slice(0, 3).map(f => f.name))
    }
  ...
  if (isHomePage) {
    handleOpenSettingsInternal = () => {
      Navigation.present({
        element: <SettingsPage settings={settings!} onUpdateSettings={(updates) => {
      ...
      toolbar={
        <Toolbar>
          ...
          <ToolbarItem placement="topBarTrailing">
            <ToolbarMenu key={'tm-' + (effectiveCopiedPath ? '1' : '0')}
              selectMode={{
                enabled: true,
              ...
              extraItems={
                <Group key={'cp-' + (effectiveCopiedPath ? '1' : '0')}>
                  {effectiveCopiedPath ? <>
                    <Divider />
                    <Button title="粘贴到当前目录" systemImage="arrow.right.doc.on.clipboard" action={async () => {
                      try {
              ...
              bottomItem={
                <Button title="设置" systemImage="gearshape" action={isHomePage ? handleOpenSettingsInternal : (settings && onSettingsChange ? () => {
                  Navigation.present({
                    element: <SettingsPage settings={settings!} onUpdateSettings={(updates) => {
                      const newSettings = { ...settings, ...updates } as AppSettings
                      saveSettings(newSettings)
  ...
  return isHomePage ? (
    <ZStack
      frame={{ maxWidth: 'infinity', maxHeight: 'infinity' }}
    >
      <NavigationStack path={homeNavPath} onDrop={{
        types: DROP_ACCEPTED_TYPES,
        validateDrop: (info) => {
          const ok = info.hasItemsConforming(DROP_ACCEPTED_TYPES);
