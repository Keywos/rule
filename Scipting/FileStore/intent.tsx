...
async function run() {
  ...
  if (!files || files.length === 0) { Script.exit(); return }
  const path = files[0]
  const ext = Path.extname(path)
  const category = getFileCategory(ext)

  const prefix = await resolveOpenerForFile(path, category)
  if (!prefix) { Script.exit(); return }

  if (prefix === 'editor:') {
    let fileSize = 0
    try { fileSize = (await FileManager.stat(path)).size } catch {}
    let content: string | undefined = undefined
    try {
      content = await FileManager.readAsString(path)
    } catch {
      for (const enc of ["utf8", "utf-16", "ascii"] as const) {
        try { content = await FileManager.readAsString(path, enc); if (content && content.length > 0) break } catch {}
      }
      if (!content) {
        try { content = await FileManager.readAsString(path, 'utf-8') } catch {}
      }
    }
